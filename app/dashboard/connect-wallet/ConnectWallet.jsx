    ...
    <div className="w-full max-w-6xl mx-auto px-4 py-12">
      <div className="text-center mb-10 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600">
          Select a wallet and connect securely by scanning a QR code or using a recovery phrase.
        </p>
      </div>
      ...
      {/* Main Modal */}
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" style={{ display: "flex" }}>
          <div
            aria-modal="true"
            role="dialog"
            tabIndex={-1}
          ...
          >
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <div className="text-blue-600 font-semibold cursor-pointer" onClick={handleCloseModal}>
                Back
              </div>
            </div>
            <div className="p-6">
              <div className="flex flex-col items-center">
                <div className="mb-4">
                  <img src={selectedWallet?.icon} alt="Icon" style={{ width: 40, height: 40 }} />
                </div>
                ...
                <button
                  className="flex items-center justify-between w-full bg-blue-50 hover:bg-blue-100 rounded p-3 mt-2"
                  id={`connect-${selectedWallet?.name?.toUpperCase()}`}
                >
                  <div>
                    <div className="font-semibold" style={{ color: "#E8831D" }}>{selectedWallet?.name}</div>
                    <div className="text-xs text-gray-600">Easy-to-use browser extension.</div>
                  </div>
                  <div>
                    <img src={selectedWallet?.icon} alt="Icon" style={{ width: 24 }} />
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full mr-10">
                  <h3 id="modal-headline" className="text-lg leading-6 mt-2 font-medium text-gray-900 flex flex-row items-center">
                    <img src={selectedWallet?.icon} style={{ marginRight: 20, height: 40, width: 40 }} alt="wallet" />
                    <span>
                      Import your <span>{selectedWallet?.name}</span> wallet
                    </span>
                  </h3>
                  <div className="mt-2">
                    <div className="mt-10">
                      <div className="flex justify-evenly mb-4">
                        {importTabs.map(tab => (
                          <div
                            key={tab.key}
                            className={`p-2 border-b-2 cursor-pointer ${activeTab === tab.key ? "border-blue-600" : "border-white hover:border-blue-600"}`}
                      ...
                      {activeTab === "phrase" && (
                        <form onSubmit={handleManualSubmit}>
                          ...
                          <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded py-2 flex items-center justify-center">
